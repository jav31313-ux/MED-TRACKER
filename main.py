...
# Configuración para ejecutables y rutas de datos
def get_data_dir():
    """Obtiene el directorio donde guardar los datos de la aplicación"""
    if hasattr(sys, '_MEIPASS'):
        # Estamos ejecutando desde un ejecutable PyInstaller
        # Usar directorio del usuario para datos persistentes
        import os
        app_data_dir = os.path.join(os.path.expanduser("~"), "MedicamentosTracker")
        if not os.path.exists(app_data_dir):
            os.makedirs(app_data_dir)
        return app_data_dir
    else:
        # Desarrollo normal - usar directorio actual
        return os.getcwd()
...
try:
    ...
    # Usar tamaño por defecto si hay problemas

# Rutas de archivos de datos - ahora en directorio persistente
DATA_DIR = get_data_dir()
USUARIO_FILE = os.path.join(DATA_DIR, "usuario.json")
MEDICAMENTOS_FILE = os.path.join(DATA_DIR, "medicamentos.json")
HISTORIAL_FILE = os.path.join(DATA_DIR, "historial_notificaciones.json")
CHECKLIST_FILE = os.path.join(DATA_DIR, "checklist_diario.json")

print(f"Directorio de datos: {DATA_DIR}")
print(f"Archivos de datos se guardarán en: {DATA_DIR}")
...
class TrackerApp(App):
    def build(self):
        try:
            ...
            # Definir el contenido KV directamente en el código
            kv_content = '''
ScreenManager:
    LoginScreen:
        name: "login"
...
'''
            ...
            # Cargar la interfaz desde el string KV
            screen_manager = Builder.load_string(kv_content)
            ...
            # Crear interfaz mínima de emergencia
            from kivy.uix.label import Label
            return Label(text=f"Error cargando la aplicación:\n{str(e)}")


if __name__ == "__main__":
    try:
        print("Iniciando Tracker de Medicamentos...")
        app = TrackerApp()
        app.run()
    except Exception as e:
